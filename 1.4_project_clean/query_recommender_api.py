...
def log_write_operation(func):
    """Implementation of monitoring and logging for all write and data sending operations."""

    def wrapper(self, *args, **kwargs):
        if func.__name__ == "write":
            query = self.query
            logging.info(
                f"Executing write operation: {func.__name__} with query: {query}"
            )
        elif func.__name__ == "recommender_send_data":
            data = self.outputs
            logging.info(
                f"Executing send data operation: {func.__name__} with data: {data}"
            )
        else:
...
class QueryRecommender_api:
    ...
    def write(self, query: str) -> None:
        ...
        session = self.session_factory()
        try:
            session.execute(text(self.query))
            session.commit()
